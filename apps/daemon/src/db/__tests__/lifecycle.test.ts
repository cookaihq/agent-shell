...
describe('持久化生命周期（项目 → 会话 → 用量）', () => {
  it('建项目 → 项目内建会话 → 存用量 → 查回；resume 指针 + 失败状态', () => {
  ...
  it('一个项目多会话可混用引擎；两项目数据互不串', () => {
    ...
    createSession(db, { projectId: p1.id, engine: 'claude', model: 'opus' })
    createSession(db, { projectId: p1.id, engine: 'codex', model: 'gpt-5.5' })
